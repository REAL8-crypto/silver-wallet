...
const REAL8 = {
  ...
  totalSupply: 8888888888 // Fixed total supply - corrected value
};
...
export function useReal8Stats(): Stats {
  ...
  const poll = useCallback(async () => {
    try {
      ...
      console.log('Fetching asset data from:', assetUrl); // Debug log
      
      const aRes = await fetch(assetUrl, { cache: 'no-store', mode: 'cors' });
      if (aRes.ok) {
        const aJson = await aRes.json();
        console.log('Asset API response:', aJson); // Debug log
        ...
        if (record?.amount) {
          ...
          if (!Number.isNaN(amount)) {
            circulating = amount;
            console.log('Circulating supply found:', circulating); // Debug log
          }
        } else {
          console.log('No amount found in asset record:', record); // Debug log
        }
      } else {
        console.log('Asset API request failed:', aRes.status, aRes.statusText); // Debug log
      }
      ...
      console.error('Error in useReal8Stats polling:', e); // Debug log
